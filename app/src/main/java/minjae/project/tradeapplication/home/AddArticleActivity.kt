...
import minjae.project.tradeapplication.databinding.ActivityAddArticleBinding
import minjae.project.tradeapplication.gallery.GalleryActivity
import minjae.project.tradeapplication.photo.CameraActivity
import minjae.project.tradeapplication.photo.PhotoListAdapter
...
class AddArticleActivity : AppCompatActivity() {
    ...
    //사진 여러장이면 오래걸리므로 코루틴으로 사진 업로드 동시진행 시킨다.
    private suspend fun uploadPhoto(uriList: List<Uri>) = withContext(Dispatchers.IO) {
        val uploadDeferred: List<Deferred<Any>> = uriList.mapIndexed { index, uri ->
            //async는 lunch와 비슷하지만 차이점으로 어떠한 값 반환해준다.
    ...
    private fun startCameraScreen() {
        ...
        startActivityForResult(CameraActivity.newIntent(this), CAMERA_REQUEST_CODE)
    }
